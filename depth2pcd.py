...
parser = argparse.ArgumentParser(
    description="Numpy depth maps of KITTI dataset to PCD format"
)
...
def save_point_cloud(filename, pc):
    ...
    for i in range(len(pts)):
        fout.write(
            "{:E} {:E} {:E}\n".format(
                Decimal(pts[i][0]), Decimal(pts[i][1]), Decimal(pts[i][2])
            )
        )
    fout.close()
...
for k in range(num_seq):
    ...
    for i in range(dimg.shape[0]):
        for j in range(dimg.shape[1]):
            pts.append(
                [
                    ratio * i * 1.0 / dimg.shape[0] - 0.5,
                    j * 1.0 / dimg.shape[1] - 0.5,
                    dimg[i][j],
                ]
            )
