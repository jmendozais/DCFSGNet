...
def disp_net(opt, dispnet_inputs):
    ...
    if opt.dispnet_encoder == "vgg":
        ...
        return build_resnet50(
            dispnet_inputs, get_disp_resnet50, is_training, "depth_net"
        )
                ...
                for i in range(opt.batch_size):
                    df.append(
                        tf.concat(
                            [
                                tf.expand_dims(
                                    conv_dn[j * opt.batch_size + i, :, :, :], axis=0
                                )
                                for j in range(opt.seq_length)
                            ],
                            axis=3,
                        )
                    )
                ...
                for i in range(opt.batch_size):
                    df.append(
                        tf.concat(
                            [
                                tf.expand_dims(
                                    concat_dn[j * opt.batch_size + i, :, :, :], axis=0
                                )
                                for j in range(opt.seq_length)
                            ],
                            axis=3,
...
def pose_net(opt, posenet_inputs):
    ...
    with tf.variable_scope("pose_net") as sc:
        with slim.arg_scope(
            ...
            pose_pred = slim.conv2d(
                conv7, 6 * opt.num_source, 1, 1, normalizer_fn=None, activation_fn=None
            )
            pose_avg = tf.reduce_mean(pose_pred, [1, 2])
            pose_final = 0.01 * tf.reshape(pose_avg, [-1, opt.num_source, 6])
            return pose_final
...
def get_disp_vgg(x):
    disp = (
        DISP_SCALING_VGG
        * slim.conv2d(x, 1, 3, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
        + 0.01
    )
    return disp
...
def get_disp_resnet50(x):
    disp = (
        DISP_SCALING_RESNET50
        * conv(x, 1, 3, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
        + 0.01
    )
    return disp
...
def get_flow(x):
    ...
    flow = FLOW_SCALING * slim.conv2d(
        x, 2, 3, 1, activation_fn=None, normalizer_fn=None
    )
    return flow
