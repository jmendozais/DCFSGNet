...
def main():
    ...
    #np.save('gt_eigen_depths.npy', gt_depths)
    #np.save('eigen_imgs.npy', imgs)
    ...
    for i in range(num_test):    
        ...
        # Scale matching
        #print("max gt", np.max(gt_depth), 'max pred', np.max(pred_depth))
        #print("masked: max gt", np.max(gt_depth[mask]), 'max pred', np.max(pred_depth[mask]))
        scalor = np.median(gt_depth[mask])/np.median(pred_depth[mask])
        #print('scalor', scalor, 'sum', np.sum(mask), 'gt sum', np.sum(gt_depth[mask]), 'pred sum', np.sum(pred_depth[mask]))
    ...
    if args.log:
        ...
        np.save(os.path.join(out_dir, 'error.npy'), errors)

    #print("{:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}".format('abs_rel', 'sq_rel', 'rms', 'log_rms', 'd1_all', 'a1', 'a2', 'a3'))
    print("{:d}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}".format(args.iter, abs_rel.mean(), sq_rel.mean(), rms.mean(), log_rms.mean(), d1_all.mean(), a1.mean(), a2.mean(), a3.mean()))    
