...
parser.add_argument(
    "--dataset_dir", type=str, help="path to kitti scene flow multiview dataset"
)
...
parser.add_argument(
    "--seq_length", type=int, default=3, help="sequence length of pose snippets"
)
    ...
    with open(os.path.join(args.dump_root, "test_flow.txt"), "w") as tf:
        for i in frame_list:
            ...
            for o in range(-half_offset, half_offset + 1):
                curr_img_path = args.dataset_dir + "training/image_%s/%.6d_%.2d.png" % (
                    args.cam_id[-1],
                    i,
                    10 + o,
                )
