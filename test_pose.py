...
def test_pose(opt):
    ...
    if opt.log:
        calib_filename = os.path.join(
            opt.dataset_dir, "sequences", "%.2d" % opt.pose_test_seq, "calib.txt"
        )
        ...
        model = Model(opt, tgt_image, src_image_stack, None)
        fetches = {"pose": model.pred_poses}
    ...
    ##### load time file #####
    with open(
        opt.dataset_dir + "sequences/%.2d/times.txt" % opt.pose_test_seq, "r"
    ) as f:
        times = f.readlines()
    ...
    with tf.Session() as sess:
        ...
        for tgt_idx in range(max_src_offset, N - max_src_offset, opt.batch_size):
            ...
            inputs = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 3 * opt.seq_length),
                dtype=np.uint8,
            )
...
def load_image_sequence(
    dataset_dir, frames, tgt_idx, seq_length, img_height, img_width
):
