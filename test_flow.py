...
def test_flow(opt):
    ...
    tgt_image_uint8 = tf.placeholder(
        tf.uint8, [opt.batch_size, opt.img_height, opt.img_width, 3], name="tgt_input"
    )
    src_image_stack_uint8 = tf.placeholder(
        tf.uint8,
        [opt.batch_size, opt.img_height, opt.img_width, opt.num_source * 3],
        name="src_stack_input",
    )
    intrinsics = tf.placeholder(
        tf.float32, [opt.batch_size, 3, 3], name="intrinsics_input"
    )
    ...
    bs = opt.batch_size
    model = Model(opt, tgt_image_uint8, src_image_stack_uint8, intrinsics_ms)
    fetches = {}
    fetches["pred_flow"] = model.fwd_full_flow_pyramid[0][
        bs * src_id : bs * (src_id + 1)
    ]
    ...
    with tf.Session(config=config) as sess:
        ...
        for tgt_idx in range(img_num):
            ...
            pred = sess.run(
                fetches,
                feed_dict={
                    tgt_image_uint8: tgt_image[None, :, :, :],
...
def unpack_image_sequence(image_seq, img_height, img_width, num_source):
    ...
    src_image_stack = np.dstack(
        [
            src_image_seq[:, i * img_width : (i + 1) * img_width, :]
