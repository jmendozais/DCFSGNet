...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        if opt.model_selection:
            file_list = self.format_file_list(opt.dataset_dir, "train_train")
            print("Loading file list for validation")
            raise Exception("model selection mode")
        else:
            file_list = self.format_file_list(opt.dataset_dir, "train")
            print("Loading file list for training")
    ...
    def make_intrinsics_matrix(self, fx, fy, cx, cy):
        ...
        r3 = tf.constant([0.0, 0.0, 1.0], shape=[1, 3])
    ...
    def data_augmentation(self, im, intrinsics, out_h, out_w):
        ...
        def random_coloring(im):
            ...
            color_image = tf.stack(
                [white * random_colors[i] for i in range(in_c)], axis=3
            )
            im_aug *= color_image
    ...
    def format_file_list(self, data_root, split):
        ...
        image_file_list = [
            os.path.join(data_root, subfolders[i], frame_ids[i] + ".jpg")
            for i in range(len(frames))
        ]
        cam_file_list = [
            os.path.join(data_root, subfolders[i], frame_ids[i] + "_cam.txt")
            for i in range(len(frames))
        ]
        all_list = {}
    ...
    def unpack_image_sequence(self, image_seq, img_height, img_width, num_source):
        ...
        src_image_1 = tf.slice(
            image_seq, [0, 0, 0], [-1, int(img_width * (num_source // 2)), -1]
        )
